...
* Hint Text(`message`)<br>
  This can be a plaintext(`%+zoom% ZOOM VIEW`) or a localization key(like `#Valve_Hint_Zoom`).<br>
  If it is a localization key, it will be translated to the user's language. After that, translatable keybinds like `%+zoom%` will be translated.<br>
